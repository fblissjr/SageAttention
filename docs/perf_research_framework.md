...
2. **Per-call instrumentation logs.** A one-shot log at install time (`[INFO] patched N blocks with kernel X`, `[WARN] could not extract required attribute on a patched block`) tells you the patch attempt happened and which fork it took. Per-call logging is finer-grained but noisier and only useful when you suspect a subset of calls is misbehaving. **Every fallback path needs a log line, and every fallback log line must carry the underlying error's message (not just its class name).** Five distinct ways this rule gets violated, all functionally equivalent for debugging utility:
   ...
   **(f) The log line is emitted, correctly, and never reaches you.** All of (a)-(e) are about whether the line gets *written*. This one is about the transport, and it defeats the whole rung without touching the code under test. **Any non-tty stage between a long-running process and the observer buffers by default**, and until you have checked one, its emptiness means nothing. Four separate instances on 2026-08-06 alone, three of them read as "the thing under test never ran":

   - File redirection (`./start.sh > log 2>&1`). Block-buffered at 4-8 KB, and the boundary landed mid model-load, so every engagement line sat unflushed while the file showed a clean startup. Cost three restarts.
   - A pipe to `tail`, which cannot emit until the pipe closes -- so a long-running script's per-arm progress was invisible for its entire runtime *despite* `flush=True` on every print. The flush was real; the pipe still held it.
   - Same shape for `head`, and for `grep` without `--line-buffered`.
