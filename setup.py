...
if not SKIP_CUDA_BUILD:
    ...
    if has_capability(("8.0", "8.6", "8.7", "8.9")):
        ext_modules.append(
            CUDAExtension(
                ...
                extra_compile_args={
                    "cxx": CXX_FLAGS,
                    # SM80 kernel is forward-compatible to Ada (8.9) and Ampere
                    # variants (8.6/8.7) via CUDA backward-compat, so we build
                    # one SM80 binary and let the driver run it on any of them.
