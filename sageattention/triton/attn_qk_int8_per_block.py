...
# Autotune over launch-config knobs only (num_warps, num_stages).
# BLOCK_M and BLOCK_N are locked by the separate per-block int8
# quantization step in sageattention/quant.py (BLKQ=128, BLKK=64);
# changing them here without matching quant changes would misalign the
# Q/K scale tables. Key covers the runtime shape so each (qo_len, kv_len)
# tuple tunes independently. Triton caches the result on disk, so only
# the first call with a new key pays the warmup cost.
@triton.autotune(
    configs=[
    ...
    key=['qo_len', 'kv_len', 'HEAD_DIM', 'BLOCK_M', 'BLOCK_N'],
)
@triton.jit
...
def forward(q, k, v, q_scale, k_scale, tensor_layout="HND", attn_mask=None, output_dtype=torch.float16, return_lse=False):
    ...
    # num_warps and num_stages are chosen by @triton.autotune on _attn_fwd.
    # First call with a new (qo_len, kv_len, HEAD_DIM, BLOCK_M, BLOCK_N)
    # tuple pays a one-time warmup; subsequent calls hit the triton cache.
    _attn_fwd[grid](
